...
from app.mcp.backend_mcp_server import mcp, inspect_ranger_state, query_trino_readonly
...
def test_ingestion_runner_has_no_internal_periodic_scheduler() -> None:
    ...
    runner_path = Path(__file__).parents[3] / "infrastructure" / "docker" / "metadata-ingestion" / "runner.py"
    content = runner_path.read_text(encoding="utf-8")

    assert "def start_scheduler" not in content, "runner.py must not contain start_scheduler"
    assert "def scheduler_loop" not in content, "runner.py must not contain scheduler_loop"
...
def test_ranger_inspection_service_calls_correct_adapter_contracts() -> None:
    with patch("app.services.ranger_inspection.create_ranger_policy_client") as mock_policy_factory, \
         patch("app.services.ranger_inspection.create_ranger_tag_store_client") as mock_tag_factory:
        ...
        mock_tag_client.list_tag_definitions.return_value = [
            {"name": "PII.Phone"}
        ]
...
def test_ranger_inspection_service_raises_on_broken_wiring() -> None:
    with patch("app.services.ranger_inspection.create_ranger_policy_client") as mock_policy_factory:
        mock_policy_client = MagicMock()
        mock_policy_client.list_policies.side_effect = ExternalSystemError("Ranger down", system="ranger")
        mock_policy_factory.return_value = mock_policy_client
...
def test_trino_diagnostic_service_does_not_limit_show_describe_explain() -> None:
    ...
    # Verify limit is added to SELECT without limit clause
...
def test_mcp_policy_placeholders_removed_from_backend_server() -> None:
    ...
    assert "inspect_policy_state" not in tools, "inspect_policy_state must be removed"
    assert "submit_policy_command" not in tools, "submit_policy_command must be removed"
