...
class ClassificationService:
    """Deterministic classification over the active DB-backed rule set."""
    ...
    def classify(
        ...
        if (
            action
            == ClassificationAction.AGENT_FALLBACK
        ):
            logical = hashlib.sha256(
                (
                    f"{event.event_id}|"
                    f"{event.entity_fqn}|"
                    f"{self.settings.agent_graph_version}"
                ).encode()
            ).hexdigest()
...
class AgentClassificationResultService:
    """Validate Agent output against the active governed tag allow-list."""
    ...
    def accept(
        self,
        request: AgentClassificationEventRequest,
    ) -> dict:
        ...
        disallowed = sorted(
            proposed_tags - allowed_tags
        )

        if disallowed:
            raise ConfigurationError(
                "Agent proposed tags outside "
        ...
        self.audit.record(
            actor_id=(
                f"bot:"
                f"{self.settings.openmetadata_agent_bot_name}"
            ),
            actor_name=(
                self.settings.openmetadata_agent_bot_name
            ),
...
AgentClassificationIntakeService = (
