...
def _minimum_confidence(
    ...
    return min(
        (item.confidence for item in suggestions),
        default=None,
    )


class ClassificationService:
    """Deterministic metadata classification with optional Agent fallback."""
    ...
    def classify(
        ...
        engine = ClassificationRuleEngine.from_path(
            self.settings.resolve_path(
                self.settings.classification_rules_path
            )
        )
        ...
        if action == ClassificationAction.AGENT_FALLBACK:
            logical = hashlib.sha256(
                (
                    f"{event.event_id}|{event.entity_fqn}|"
...
class AgentClassificationResultService:
    """Validate Agent output and enqueue native OpenMetadata Suggestions."""
    ...
    def accept(
        ...
        proposed_tags = {
            item.tag for item in request.suggestions
        }
        disallowed = sorted(proposed_tags - allowed_tags)
        if disallowed:
            raise ConfigurationError(
                "Agent proposed tags outside the governed allow-list: "
                + ", ".join(disallowed)
            )
        ...
        self.audit.record(
            actor_id=(
                f"bot:{self.settings.openmetadata_agent_bot_name}"
            ),
