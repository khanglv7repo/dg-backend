...
class Settings(BaseSettings):
    ...
    openmetadata_execution_bot_name: str = (
    ...
    openmetadata_auto_tag_bot_token: (
        SecretStr | None
    ) = Field(
        default=None,
        validation_alias=AliasChoices(
            "OM_AUTO_TAG_BOT_TOKEN"
        ),
    )
    ...
    agent_graph_version: str = (
        "classification-graph-v1"
    )
    agent_prompt_version: str = (
        "classification-prompt-v1"
    )
    openmetadata_agent_bot_name: str = (
        "governance-agent-bot"
    )
    openmetadata_mcp_url: str = (
        "http://localhost:8585/mcp"
    )
    openmetadata_agent_bot_token: (
        SecretStr | None
    ) = None
    ...
    ranger_tag_store_base_url: str = (
        "http://localhost:6080/service/tags"
    )
    ...
    def validate_machine_identity_separation(
        ...
        if (
            self.openmetadata_execution_bot_name
            == self.openmetadata_agent_bot_name
        ):
            raise ValueError(
                "OpenMetadata execution and agent bots "
                "must be different machine identities"
            )
